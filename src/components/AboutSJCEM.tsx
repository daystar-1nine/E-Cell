    ...
    <section id="about-sjcem" ref={sectionRef} className="relative min-h-[100dvh] w-full flex items-center py-24 lg:py-32 z-10 overflow-hidden">
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col lg:flex-row-reverse gap-10 lg:gap-16 items-center">
          ...
          {/* Text Content */}
          <motion.div 
            className="w-full lg:w-1/2 bg-[var(--color-surface)] p-6 md:p-8 rounded-lg border border-hairline"
            initial="hidden"
            whileInView="visible"
          ...
          >
            ...
            <motion.h2 variants={itemVariants} className="text-display-md lg:text-display-lg font-bold font-inter text-[var(--color-text-main)] mb-6 md:mb-8 leading-tight">
              A premier autonomous institute.
            </motion.h2>
            
            <div className="space-y-4 md:space-y-6 text-base md:text-lg text-[var(--color-text-muted)] font-inter leading-relaxed mb-8 md:mb-10">
            ...
            {/* Credential Badges */}
            <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mt-8 border-t border-hairline pt-8">
              {credentials.map((cred, idx) => (
                <div key={idx} className="flex items-center gap-4 bg-[var(--color-surface-elevated)] border border-hairline p-3 md:p-4 rounded-md transition-colors hover:border-hairline-strong group">
                  <div className="p-2.5 bg-[var(--color-background)] rounded-md text-[var(--color-primary)] transition-colors shrink-0 border border-hairline">
                    <cred.icon className="w-5 h-5 md:w-6 md:h-6" />
                  </div>
                  <span className="text-sm md:text-base font-semibold text-[var(--color-text-main)] font-inter leading-tight">{cred.label}</span>
                </div>
              ))}
            </motion.div>
          </motion.div>
          ...
          <div className="w-full lg:w-1/2" style={{ perspective: "1000px" }}>
            <motion.div 
              style={{ y, rotateX }}
              className="relative w-full aspect-[4/3] rounded-lg overflow-hidden border border-hairline bg-[var(--color-surface)] flex items-center justify-center group"
            >
              <img src="/images/campus_photo.png" alt="SJCEM Campus" className="w-full h-full object-cover z-0 group-hover:scale-105 transition-transform duration-700 opacity-80 mix-blend-luminosity" />
