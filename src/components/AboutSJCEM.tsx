...
export default function AboutSJCEM() {
  ...
  return (
    <section id="about-sjcem" ref={sectionRef} className="relative min-h-screen w-full flex items-center py-32 z-10 overflow-hidden">
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col lg:flex-row-reverse gap-16 items-center">
          
          {/* Text Content */}
          <motion.div 
            className="w-full lg:w-1/2 dynamic-scrim p-8 rounded-3xl backdrop-blur-sm border border-white/10 shadow-xl"
            initial="hidden"
            whileInView="visible"
          ...
          >
            ...
            <motion.h2 variants={itemVariants} className="text-4xl md:text-5xl font-bold font-inter text-white mb-8 leading-tight dynamic-text-shadow">
              A premier autonomous institute.
            </motion.h2>
            
            <div className="space-y-6 text-lg text-slate-200 font-outfit leading-relaxed mb-10">
            ...
            <motion.div variants={itemVariants} className="flex flex-wrap gap-3">
              {credentials.map((cred, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-slate-900/40 backdrop-blur-md border border-white/10 px-4 py-2 rounded-full shadow-sm hover:bg-slate-900/60 transition-colors">
                  <cred.icon className="w-4 h-4 text-sky-400" />
                  <span className="text-sm font-medium text-white">{cred.label}</span>
                </div>
              ))}
            </motion.div>
          </motion.div>
          ...
          <div className="w-full lg:w-1/2" style={{ perspective: "1000px" }}>
            ...
            >
              ...
              <div className="absolute inset-0 border-[10px] border-white/20 rounded-3xl z-10 m-4"></div>
              <Building2 className="w-24 h-24 text-slate-400 opacity-50 z-20 group-hover:scale-110 transition-transform duration-700" />
              <div className="absolute bottom-10 z-20 text-slate-500 font-inter font-medium tracking-wide uppercase text-sm">
